...
4. **四大核心課程 (消防、緊急、感控、性別)**：
   ...
   * 四科總分累計須 **$\ge 10$ 分**。
5. **多元與原住民文化課程**：
   * **舊制**（民國 113/06/03 前）：「原住民族與多元族群文化」為單一合併類別，**要求累積達 2 分**。
     * 注意：這個 2 分是**要求門檻**，不是總分上限。這些積分是否計入 120 分總分，一律由該課程的**課程屬性**決定（與其他課程相同）；`culturalOldCapped = min(累積, 2)` 只是達成度顯示，不參與總分計算。
